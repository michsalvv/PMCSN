...
typedef struct
{
    ...
    int status;          // event status, {1=busy, 0=idle}
 ...
 */
  ...
  while (event[i].status == 0)       /* find the index of the first 'active' element in the event list */
    i++;                        
  e = i;                        
  while (i < SERVERS) {         /* now, check the others to find which event type is most imminent */
    i++;                      
   ...
   int FindOne(event_list event)
{
  ...
  while (event[i].status == 1)       /* find the index of the first available (idle) server*/
    i++;                         
  s = i;
  while (i < SERVERS) {         /* now, check the others to find which has been idle longest    */ 
    i++;                        
...
void printEventList(event_list list, int coda, double clock){
  printf("Clock: %f\n", clock);
  printf("Situazione eventi: {");
  for (int i = 0; i <= SERVERS; i++){
    printf("%f  ", list[i].t);   
  }
  printf("}\n");
  printf("Numero di job in coda: %d\n", coda);
}
...
/* 
All'indice 0 c'è l'arrivo. 
I successivi m indici riguardano i server
*/
  clock.current = START;
...
// Processamento di un arrivo
    if (e == 0) {                                     // Se il prossimo evento individuato è all'indice 0 nell'event list, allora è un arrivo
    printf("--> Arrivo \n\n");
      ...
      if (number <= SERVERS) {                      // Se numero di job in servizio è minore del numero di server allora c'è un server libero (minore uguale perchè già è stato incrementato)
        ...
        event[s].status      = 1;
        printf("Ci sono serventi liberi quindi è possibile generare un tempo di servizio... %f\n", service);
        printf("Il completamento avverrà all'istante: (%f + %f) = %f, dal servente #%d\n", clock.current, service, event[s].t, s);
      }
    }
// Processamento di un completamento
      ...
      number--;
      s                 = e;                        
      if (number >= SERVERS) {                          // Se c'è un job in coda ma non ci sono serventi liberi      
